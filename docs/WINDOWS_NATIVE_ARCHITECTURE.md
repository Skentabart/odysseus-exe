...
## Implementation progress: local search, diagnostics, AI runtime, MCP

The second implementation slice expands the MVP scaffolding with concrete replacement seams for services that previously depended on containerized infrastructure:

- `windows_native.search.SQLiteFtsSearch` provides an embedded SQLite FTS5 search backend for baseline document and memory lookup without OpenSearch or a ChromaDB server. Semantic vectors still require a later embedded vector backend.
- `windows_native.diagnostics.export_diagnostics` creates a ZIP containing redacted config and logs while intentionally excluding models, documents, uploads, memory, and cache.
- `windows_native.ai_runtime.LlamaServerManager` validates `llama-server.exe` and `.gguf` model paths, builds a loopback-only launch command, and exposes health polling for local model runtime startup.
- `windows_native.mcp.McpProcessManager` starts configured local MCP servers as child processes and stops them during shutdown, preserving MCP without Docker.

## Implementation progress: model registry and installer scaffold

The third implementation slice adds two more concrete Windows-native seams:

- `windows_native.models.ModelRegistry` lists local `.gguf` models, reports size metadata, validates user-selected GGUF files, and refuses to delete files outside the managed model directory.
- `windows_native.hardware` now has a Windows PowerShell/CIM GPU probe path for NVIDIA/AMD/Intel names and AdapterRAM, plus CUDA detection via `nvidia-smi` when available.
- `packaging/inno/Odysseus.iss` defines the first installer script, while `build_windows.ps1` now assembles an app layout, a portable layout, and conditionally invokes Inno Setup without faking `Odysseus-Setup.exe` when the compiler is unavailable.

## Implementation progress: maintenance CLI and smoke backend

The fourth implementation slice adds a small maintenance CLI (`python -m windows_native`) that can initialize the data layout, print hardware recommendations, list managed GGUF models, and export diagnostics. A standard-library smoke backend is also available for launcher health-check tests. These are developer/MVP tools only; they do not replace the required final Tauri/WebView2 `Odysseus.exe` desktop shell.

## Implementation progress: runtime orchestration and migrations

The fifth implementation slice adds `WindowsNativeRuntime`, which coordinates local storage, SQLite FTS search, SQLite migrations, model registry, MCP process lifecycle, backend launch, AI runtime launch, and log configuration from one object. A minimal SQLite migration runner creates native metadata tables idempotently in the user data database.
