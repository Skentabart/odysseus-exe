...
Until the Tauri/WebView2 `Odysseus.exe` shell is added, the portable scaffold includes `Odysseus.cmd` for maintenance-only initialization and hardware diagnostics. This is not the final end-user launcher.

```powershell
python -m windows_native --app-dir . --portable init
python -m windows_native --app-dir . --portable hardware
python -m windows_native --app-dir . --portable list-models
python -m windows_native --app-dir . --portable export-diagnostics
```
